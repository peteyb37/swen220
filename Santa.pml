 ...
 */
#define NELVES  3 //SHOULD BE 3 for sake of testing it is 1
#define NTOYS   3
...
proctype Elf(byte id) {
	byte numToys = 0 ; 						//total number of toys
	chan from_santa = [1] of { mtype } ;	//channel from Santa to Elf
	mtype response ;						//mtype for receiving from Santa
 ...
 */
proctype Santa() {
	byte elvesGone = 0 ;		//Number of elves gone
	byte numTrains = 0 ;		//number of trains assigned
	byte numDolls = 0 ;			//number of dolls assigned
	byte numBalls = 0 ;			//number of balls assigned
	mtype assigned[NELVES] ;	//array for assigned variables
	byte id ;					//elf id
	mtype ToySelected ;			//mtype for the selected toy to be sent to elf
	chan from_santa ;			//channel for from Santa to Elf
	mtype response ;			//mtype for receiving from an elf
